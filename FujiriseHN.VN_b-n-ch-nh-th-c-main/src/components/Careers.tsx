...
import { CAREERS as DEFAULT_CAREERS, getCachedSettings } from '../constants';
  ...
  React.useEffect(() => {
    const loadCareers = async () => {
      ...
      if (data?.content_dict?.careers && data.content_dict.careers.length > 0) {
        ...
        setCareers(DEFAULT_CAREERS as Job[]);
      }
    ...
    <section id="careers" className="py-32 bg-white relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <div className="mb-20 text-center md:text-left">
          <span className="text-sm font-bold uppercase tracking-wider text-fuji-accent mb-6 block">
            Careers at Fujirise
          </span>
          <h2 className="text-5xl md:text-7xl font-black text-fuji-blue tracking-tighter leading-tight mb-8">
            ...
            >
              <div className="flex items-start gap-8">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-6 text-[10px] font-bold uppercase tracking-wider text-slate-400 group-hover:text-white/50">
              ...
              <div className="flex items-center gap-10">
                ...
                <button 
                  ...
                  className="bg-white text-fuji-blue px-10 py-5 rounded-2xl font-bold text-xs uppercase tracking-wider shadow-xl shadow-black/5 group-hover:bg-fuji-accent group-hover:text-white transition-all flex items-center gap-3 whitespace-nowrap active:scale-95"
                >
        ...
        <div className="mt-24 p-12 bg-fuji-dark rounded-[50px] text-white flex flex-col md:flex-row items-center justify-between gap-10 relative overflow-hidden">
          ...
          <button 
            ...
            className="btn-primary bg-fuji-accent hover:bg-white hover:text-fuji-dark px-12 relative z-10 h-16 uppercase tracking-wider text-xs font-bold"
          >
            GỬI THÔNG TIN NGAY
          </button>
        </div>
              ...
              {!isSuccess ? (
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
                  <div className="space-y-2">
                    <label className="text-[10px] font-bold uppercase tracking-wider text-slate-400 flex items-center gap-2"><User size={12} /> Họ và tên *</label>
                    <input 
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] font-bold uppercase tracking-wider text-slate-400 flex items-center gap-2"><Phone size={12} /> Số điện thoại *</label>
                    <input 
                  ...
                  {formConfig.showEmail && (
                    <div className="space-y-2">
                      <label className="text-[10px] font-bold uppercase tracking-wider text-slate-400 flex items-center gap-2"><Mail size={12} /> Email liên hệ</label>
                      <input 
                        {...register('email')}
                        type="email"
                  ...
                  {formConfig.showExperience && (
                    <div className="space-y-2">
                      <label className="text-[10px] font-bold uppercase tracking-wider text-slate-400 flex items-center gap-2"><MessageSquare size={12} /> Kinh nghiệm & Link CV {formConfig.requireExperience ? '*' : '(nếu có)'}</label>
                      <textarea 
                  ...
                  <button
                    ...
                    className={cn(
                      "w-full py-5 rounded-2xl flex items-center justify-center gap-3 font-bold text-xs uppercase tracking-wider transition-all mt-4",
