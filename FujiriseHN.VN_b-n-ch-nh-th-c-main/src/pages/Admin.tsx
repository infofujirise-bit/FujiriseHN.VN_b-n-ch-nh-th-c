...
import { PRODUCTS, FAQS, CAREERS, CONTACT_INFO } from '../constants';
...
type Tab = 'dashboard' | 'content' | 'jobs' | 'users' | 'images' | 'projects' | 'settings' | 'warranty' | 'web_content' | 'configurator' | 'faq' | 'accounts' | 'news';

const TAB_TITLES: Record<Tab, string> = {
  dashboard: 'Bảng Điều Khiển',
  content: 'Quản lý Báo giá',
  users: 'Hồ sơ Ứng tuyển',
  images: 'Sản phẩm',
  projects: 'Quản lý Công trình',
  configurator: 'Mô phỏng Nội thất',
  faq: 'Hỏi & Đáp (FAQ)',
...
export default function Admin() {
  ...
  const [cropImage, setCropImage] = React.useState<{ url: string, onSave: (blob: Blob) => Promise<void> } | null>(null);

  // New state for filtering leads by status
  const [activeLeadStatusFilter, setActiveLeadStatusFilter] = React.useState<'all' | Lead['status']>('all');

  const handleTabChange = (tab: Tab, statusFilter: 'all' | Lead['status'] = 'all') => {
    setActiveTab(tab);
    setActiveLeadStatusFilter(statusFilter);
  ...
  // Hook xử lý chuông thông báo trong Web và Bắn thông báo ra màn hình máy tính
  React.useEffect(() => {
    if (!user) return;
    ...
    const loadRecent = async () => {
      ...
      if (data) { setNotifications(data); setUnreadCount(data.filter((d: Lead) => d.status === 'new').length); }
    };
    loadRecent();

    const channel = supabase.channel('global_notifications') // Kênh này đã có ở bài trước
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'leads' }, (payload: any) => {
        ...
        // Bắn thông báo ra góc phải màn hình Window/Mac
        if ("Notification" in window && Notification.permission === "granted") {
          new Notification("Yêu cầu báo giá mới", { body: `${newLead.name || 'Khách hàng'} vừa gửi biểu mẫu!`, icon: sysLogo });
        }
      })
      .subscribe();
    ...
    <div className="h-screen w-full bg-slate-50 flex overflow-hidden relative">
      ...
      <aside className={cn(
        ...
        <nav className="flex-1 overflow-y-auto px-4 space-y-1 pb-6">
          ...
          <NavBtn icon={<LayoutDashboard size={20} />} label="Dashboard" active={activeTab === 'dashboard'} onClick={() => handleTabChange('dashboard')} />
          
          <div className="h-4" />
          <p className="px-4 text-[10px] font-black uppercase text-white/30 tracking-widest mb-4">Quản lý Leads</p>
          <NavBtn icon={<FileText size={20} />} label="Leads & Báo giá" active={activeTab === 'content'} onClick={() => handleTabChange('content')} />
          <NavBtn icon={<Users size={20} />} label="Ứng tuyển" active={activeTab === 'users'} onClick={() => handleTabChange('users')} />
          
          <div className="h-4" />
          <p className="px-4 text-[10px] font-black uppercase text-white/30 tracking-widest mb-4">Danh mục</p>
          <NavBtn icon={<ImageIcon size={20} />} label="Sản phẩm" active={activeTab === 'images'} onClick={() => handleTabChange('images')} />
          <NavBtn icon={<Building size={20} />} label="Công trình" active={activeTab === 'projects'} onClick={() => handleTabChange('projects')} />
          <NavBtn icon={<Palette size={20} />} label="Mô phỏng" active={activeTab === 'configurator'} onClick={() => handleTabChange('configurator')} />
          <NavBtn icon={<HelpCircle size={20} />} label="Hỏi đáp (FAQ)" active={activeTab === 'faq'} onClick={() => handleTabChange('faq')} />
          <NavBtn icon={<Newspaper size={20} />} label="Tin tức & Sự kiện" active={activeTab === 'news'} onClick={() => handleTabChange('news')} />
          <NavBtn icon={<ShieldCheck size={20} />} label="Chính sách" active={activeTab === 'warranty'} onClick={() => handleTabChange('warranty')} />
      ...
      <main className="flex-1 flex flex-col h-full relative overflow-hidden">
        <header className="h-16 md:h-20 bg-white border-b border-slate-100 flex items-center justify-between px-4 md:px-10 shrink-0 shadow-[0_4px_20px_rgba(0,0,0,0.02)] gap-4">
          ...
          <div className="flex items-center gap-3 md:gap-6">
             ...
             <div className="relative">
               <button 
                 onClick={() => { setShowNotifMenu(!showNotifMenu); if (unreadCount > 0) setUnreadCount(0); }}
                 className="relative w-10 h-10 flex items-center justify-center text-slate-400 hover:text-fuji-blue transition-colors"
               >
                  <Bell size={20} />
                  {unreadCount > 0 && <div className="absolute top-2 right-2 w-2.5 h-2.5 bg-red-500 rounded-full border-2 border-white animate-pulse" />}
               </button>
               {showNotifMenu && (
                 <div className="absolute top-full right-0 mt-2 w-80 bg-white rounded-2xl shadow-2xl border border-slate-100 z-50 overflow-hidden">
                   <div className="p-4 border-b border-slate-50 flex justify-between items-center bg-slate-50/50">
                     <h4 className="text-xs font-black uppercase tracking-widest text-fuji-blue">Thông báo Form mới</h4>
                   </div>
                   <div className="max-h-[300px] overflow-y-auto">
                     {notifications.length === 0 ? (
                       <div className="p-8 text-center text-slate-400 text-xs font-medium italic">Chưa có thông báo nào</div>
                     ) : (
                       notifications.map((n, i) => (
                         <div key={i} onClick={() => { setShowNotifMenu(false); handleTabChange(n.message?.startsWith('Ứng') ? 'users' : 'content', 'new'); }} className="p-4 border-b border-slate-50 hover:bg-slate-50 cursor-pointer transition-colors">
                            <p className="text-xs font-bold text-slate-800 mb-1">{n.name || 'Khách vãng lai'} <span className="font-normal text-slate-500">vừa gửi yêu cầu</span></p>
                            <p className="text-[10px] text-slate-500 line-clamp-1">{n.message}</p>
                            <p className="text-[9px] text-slate-400 mt-2 font-bold text-fuji-accent">{new Date(n.created_at).toLocaleString('vi-VN')}</p>
                         </div>
                       ))
                     )}
                   </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-10">
          <AnimatePresence mode="wait">
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              transition={{ duration: 0.2 }} // Slightly reduced duration for a snappier fade
            >
              {activeTab === 'dashboard' && <Dashboard onNavigateToLeads={handleTabChange} />}
              {activeTab === 'content' && <LeadManager type="consultation" statusFilter={activeLeadStatusFilter} />}
              {activeTab === 'users' && <LeadManager type="recruitment" statusFilter={activeLeadStatusFilter} />}
              {activeTab === 'jobs' && <JobManager />}
              {activeTab === 'images' && <ProductManager setCropImage={setCropImage} />}
              {activeTab === 'projects' && <ProjectManager setCropImage={setCropImage} />}
              {activeTab === 'faq' && <FAQManager />}
              {activeTab === 'news' && <NewsManager setCropImage={setCropImage} />}
              {activeTab === 'accounts' && user?.role === 'admin' && <UserManager />}
              {activeTab === 'configurator' && <ConfiguratorManager setCropImage={setCropImage} />}
              {activeTab === 'warranty' && <WarrantyManager />}
              {activeTab === 'web_content' && <WebContentManager setCropImage={setCropImage} />}
              {activeTab === 'settings' && <SettingsManager />}
            </motion.div>
          </AnimatePresence>
        </div>
      </main>
      {cropImage && <ImageCropperModal imageUrl={cropImage.url} onSave={cropImage.onSave} onCancel={() => setCropImage(null)} />}
    </div>
  );
}
...
interface DashboardStats {
  ...
  contactedConsultation: number;
  completedConsultation: number;
  totalRecruitment: number;
  newRecruitment: number;
  contactedRecruitment: number;
  completedRecruitment: number;
}

function Dashboard({ onNavigateToLeads }: { onNavigateToLeads: (tab: Tab, statusFilter: 'all' | Lead['status']) => void }) {
  const [stats, setStats] = React.useState<DashboardStats>({ totalLeads: 0, newLeads: 0, conversion: '0.0%', completionRate: 0, totalConsultation: 0, newConsultation: 0, contactedConsultation: 0, completedConsultation: 0, totalRecruitment: 0, newRecruitment: 0, contactedRecruitment: 0, completedRecruitment: 0 });
  const [chartData, setChartData] = React.useState<{name: string, leads: number}[]>([]);
  const [isAnalyzing, setIsAnalyzing] = React.useState(false);

  React.useEffect(() => {
    const fetchDashboardData = async () => {
      const { data: leadsData } = await supabase.from('leads').select('status, created_at, message');
      ...
      if (leadsData) {
        ...
        const consultationLeads = leadsData.filter((l: Lead) => !l.message?.startsWith('Ứng tuyển:'));
        const recruitmentLeads = leadsData.filter((l: Lead) => l.message?.startsWith('Ứng tuyển:'));

        setStats({ // Cập nhật tất cả các thuộc tính của DashboardStats trong một lần
          totalLeads: total,
          newLeads: newLeads,
          conversion: `${conversionRate}%`,
          completionRate: completionRate,
          totalConsultation: consultationLeads.length,
          newConsultation: consultationLeads.filter((l: Lead) => l.status === 'new').length,
          contactedConsultation: consultationLeads.filter((l: Lead) => l.status === 'contacted').length,
          completedConsultation: consultationLeads.filter((l: Lead) => l.status === 'completed').length,
          totalRecruitment: recruitmentLeads.length,
          newRecruitment: recruitmentLeads.filter((l: Lead) => l.status === 'new').length,
          contactedRecruitment: recruitmentLeads.filter((l: Lead) => l.status === 'contacted').length,
          completedRecruitment: recruitmentLeads.filter((l: Lead) => l.status === 'completed').length,
        });
  ...
  return (
    <div className="space-y-10">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
        <DashboardStatCard icon={<MessageSquare className="text-blue-500" />} label="Tổng yêu cầu" value={stats.totalLeads} trend="Thực tế" positive={true} />
        <DashboardStatCard icon={<AlertCircle className="text-orange-500" />} label="Chờ xử lý" value={stats.newLeads} trend={stats.newLeads > 0 ? "Cần xử lý" : "An toàn"} positive={stats.newLeads === 0} />
        <DashboardStatCard icon={<Activity className="text-green-500" />} label="Tỉ lệ Chuyển đổi" value={stats.conversion} trend="Thành công" positive={true} />
        <DashboardStatCard icon={<ShieldCheck className="text-indigo-500" />} label="Hệ thống" value="Active" trend="Stable" positive={true} />
      </div>

      {/* New section for quick lead management */}
      <div className="bg-white rounded-[40px] p-8 shadow-sm border border-slate-100">
        <h3 className="font-black text-fuji-blue text-xl tracking-tighter uppercase mb-6">Quản lý Leads Nhanh</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <DashboardStatCard 
            icon={<FileText className="text-blue-500" />} 
            label="Báo giá mới" 
            value={stats.newConsultation} 
            trend={stats.newConsultation > 0 ? "Cần xử lý" : "An toàn"} 
            positive={stats.newConsultation === 0} 
            onClick={() => onNavigateToLeads('content', 'new')}
          />
          <DashboardStatCard 
            icon={<MessageSquare className="text-purple-500" />} 
            label="Báo giá đang xử lý" 
            value={stats.contactedConsultation} 
            trend={stats.contactedConsultation > 0 ? "Đang theo dõi" : "Không có"} 
            positive={stats.contactedConsultation === 0} 
            onClick={() => onNavigateToLeads('content', 'contacted')}
          />
          <DashboardStatCard 
            icon={<Users className="text-orange-500" />} 
            label="Ứng tuyển mới" 
            value={stats.newRecruitment} 
            trend={stats.newRecruitment > 0 ? "Cần xử lý" : "An toàn"} 
            positive={stats.newRecruitment === 0} 
            onClick={() => onNavigateToLeads('users', 'new')}
          />
          <DashboardStatCard 
            icon={<Briefcase className="text-green-500" />} 
            label="Ứng tuyển đã xử lý" 
            value={stats.completedRecruitment} 
            trend={stats.completedRecruitment > 0 ? "Đã xong" : "Không có"} 
            positive={stats.completedRecruitment > 0} 
            onClick={() => onNavigateToLeads('users', 'completed')}
          />
        </div>
      </div>
...
function LeadManager({ type, statusFilter = 'all' }: { type: 'consultation' | 'recruitment', statusFilter?: 'all' | Lead['status'] }) {
  ...
  const fetchLeads = React.useCallback(async () => {
    ...
    if (data) {
      let filtered = type === 'recruitment' 
        ? data.filter((l: Lead) => l.message?.startsWith('Ứng tuyển:'))
        : data.filter((l: Lead) => !l.message?.startsWith('Ứng tuyển:'));
  ...
  const deleteLead = async (id: string) => {
    if (confirm('Bạn có chắc chắn muốn xóa dữ liệu này vĩnh viễn?')) {
  ...
  const deleteAllLeads = async () => {
    ...
    if (confirm('⚠️ CẢNH BÁO: Bạn có chắc chắn muốn XÓA TOÀN BỘ (Reset) dữ liệu trong danh sách hiện tại? Hành động này KHÔNG THỂ KHÔI PHỤC!')) {
      const idsToDelete = leads.map(l => l.id);
      const { error } = await supabase.from('leads').delete().in('id', idsToDelete);
      if (!error) {
        setLeads([]);
        alert('Đã xóa (Reset) toàn bộ dữ liệu thành công!');
      } else {
  ...
  return (
    <div className="bg-white rounded-[40px] shadow-sm border border-slate-100 overflow-hidden">
      <div className="p-10 border-b border-slate-50 flex justify-between items-center">
         <div>
           <h3 className="text-lg font-black text-fuji-blue tracking-tighter uppercase">
             {type === 'consultation' ? 'Danh sách Báo giá & Katalog' : 'Danh sách Ứng viên'}
             {statusFilter !== 'all' && ` (${statusFilter === 'new' ? 'Mới' : statusFilter === 'contacted' ? 'Đang xử lý' : statusFilter === 'completed' ? 'Hoàn thành' : 'Đã hủy'})`}
           </h3>
           <p className="text-xs text-slate-400 mt-1 font-medium italic">Quản lý trạng thái và liên hệ khách hàng</p>
         </div>
         <div className="flex gap-4">
            <button onClick={fetchLeads} className="flex items-center gap-2 px-4 py-2 bg-blue-50 text-blue-600 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-blue-100 transition-all" title="Tải lại / Revert Data">
               <Activity size={14} /> Tải Lại
            </button>
            <button onClick={deleteAllLeads} className="flex items-center gap-2 px-4 py-2 bg-red-50 text-red-600 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-red-100 transition-all">
              ...
              <tr key={lead.id} className="hover:bg-slate-50/30 transition-colors">
                ...
                <td className="p-8 text-right">
                  <div className="flex justify-end gap-2">
                    ...
                    <button onClick={() => deleteLead(lead.id!)} className="w-8 h-8 rounded-lg bg-slate-50 text-slate-400 hover:bg-red-500 hover:text-white transition-all flex items-center justify-center">
                      <Trash2 size={14} />
                    </button>
                  </div>
                </td>
  ...
  return (
    <div className="space-y-8 max-w-5xl mx-auto">
      ...
      <div className="flex justify-end">
         <button onClick={saveFaqs} disabled={isSaving} className="px-10 py-4 bg-fuji-blue text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl hover:bg-fuji-accent transition-all active:scale-95 disabled:opacity-50">
           {isSaving ? 'Đang lưu...' : 'Lưu danh sách FAQ'}
         </button>
      </div>
    </div>
  );
...
function JobManager() {
  ...
  React.useEffect(() => {
    const fetchJobs = async () => {
      const { data } = await supabase.from('site_settings').select('content_dict').eq('id', 'default').single();
      if (data?.content_dict?.careers && data.content_dict.careers.length > 0) {
        setJobs(data.content_dict.careers);
      } else {
        setJobs(CAREERS as Job[]);
      }
  ...
  return (
    <div className="space-y-8 max-w-5xl mx-auto">
      ...
      <div className="flex justify-end">
         <button onClick={saveSettings} disabled={isSaving} className="px-10 py-4 bg-fuji-blue text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl hover:bg-fuji-accent transition-all active:scale-95 disabled:opacity-50">
           {isSaving ? 'Đang lưu...' : 'Lưu cấu hình tuyển dụng'}
         </button>
      </div>
    </div>
  );
...
function ProductManager({ setCropImage }: { setCropImage: any }) {
    ...
    const fetchProducts = async () => {
      ...
      if (data?.content_dict?.products && data.content_dict.products.length > 0) {
        ...
        setProducts(PRODUCTS.map((p: any) => ({
          ...p,
          category: p.category || 'Thang máy Ca bin',
          technology: 'Cáp kéo',
          ...
          cabin: {
            material: p.cabin?.material || 'Lựa chọn linh hoạt giữa inox sọc nhuyễn, inox hoa văn tinh xảo hoặc kính cường lực hiện đại, phù hợp nhiều phong cách thiết kế.\nTạo điểm nhấn đẳng cấp, nâng tầm thẩm mỹ không gian nội thất.\nChống trơn trượt, dễ vệ sinh, đảm bảo an toàn và độ bền trong quá trình sử dụng.'
          }
        })) as Product[]);
      }
    };
        ...
        {products.map(product => (
          <div key={product.id} className="bg-white rounded-[50px] p-8 shadow-sm border border-slate-100 grid md:grid-cols-5 gap-8 group">
            <div className="md:col-span-2 h-64 rounded-[40px] overflow-hidden relative">
              <img src={product.images?.[0] || product.image} className="w-full h-full object-contain bg-slate-50" />
           ...
           <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white rounded-[50px] p-12 max-w-4xl w-full max-h-[90vh] overflow-y-auto shadow-2xl relative">
            ...
            setIsSaving(true);
              ...
              setIsSaving(false);
                                        ...
                                        setCropImage({
                                          ...
                                          onSave: async (blob: Blob) => {
                                            ...
                                            if (isMock) {
                                              ...
                                              setIsEditing((prev: any) => { if (!prev) return null; const newImages = [...prev.images]; newImages[idx] = localUrl; return { ...prev, images: newImages, image: newImages[0] }; });
                                              setCropImage(null); return;
                                            }
                                            const file = new File([blob], `cropped-${Date.now()}.jpg`, { type: 'image/jpeg' });
                                            const fileName = `product-crop-${Date.now()}-${Math.random().toString(36).substring(7)}.jpg`;
                                            const { error } = await supabase.storage.from('images').upload(fileName, file);
                                            if (!error) {
                                              const { data: { publicUrl } } = supabase.storage.from('images').getPublicUrl(fileName);
                                              setIsEditing((prev: any) => { if (!prev) return null; const newImages = [...prev.images]; newImages[idx] = publicUrl; return { ...prev, images: newImages, image: newImages[0] }; });
                                            } else {
                                              alert('Lỗi upload ảnh đã cắt');
                                            }
                                            setCropImage(null);
                                          }
                                        });
                                      }} className="w-6 h-6 bg-fuji-blue text-white rounded-full flex items-center justify-center shadow-md hover:bg-fuji-accent">
                                        <CropIcon size={12} />
                                      </button>
                                      <button type="button" onClick={() => removeImage(idx)} className="w-6 h-6 bg-red-500 text-white rounded-full flex items-center justify-center shadow-md hover:bg-red-600">
                                      <X size={12} />
                                    </button>
                                    </div>
                                  </div>
                ...
                <button type="submit" disabled={isSaving} className="flex-1 py-5 rounded-2xl bg-fuji-blue text-white font-black text-[10px] uppercase tracking-widest shadow-xl shadow-fuji-blue/20 hover:bg-fuji-accent transition-all active:scale-95 disabled:opacity-50">
...
function ProjectManager({ setCropImage }: { setCropImage: any }) {
  ...
  React.useEffect(() => {
    const fetchProjects = async () => {
      ...
      if (data?.content_dict?.projects && data.content_dict.projects.length > 0) {
        ...
        const dummy = [
          { id: 1, title: "Biệt thự Tân cổ điển Vinhomes Riverside", location: "Long Biên, Hà Nội", type: "Thang máy Kính Panorama", image: "https://images.unsplash.com/photo-1600596542815-ffad4c1539a9?auto=format&fit=crop&q=80&w=1200", images: ["https://images.unsplash.com/photo-1600596542815-ffad4c1539a9?auto=format&fit=crop&q=80&w=1200"], description: "Giải pháp thang máy kính toàn cảnh không hố pit, tôn vinh vẻ đẹp kiến trúc tân cổ điển." },
          { id: 2, title: "Penthouse Đảo Kim Cương", location: "Quận 2, TP. HCM", type: "Thang máy Inox Gương Vàng", image: "https://images.unsplash.com/photo-1600607687931-cebf004f14e3?auto=format&fit=crop&q=80&w=1200", images: ["https://images.unsplash.com/photo-1600607687931-cebf004f14e3?auto=format&fit=crop&q=80&w=1200"], description: "Kiệt tác mạ vàng PVD đẳng cấp, hài hòa với không gian nội thất xa hoa của giới tinh hoa." }
        ];
        setProjects(dummy);
      }
    };
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const slotsLeft = 10 - currentImages.length;
    const filesToUpload = files.slice(0, slotsLeft);
    if (files.length > slotsLeft) alert(`Chỉ thêm tối đa 10 ảnh.`);
  ...
  const removeImage = (idx: number) => {
    setIsEditing((prev: any) => {
      if (!prev) return null;
      const updated = prev.images.filter((_: any, i: number) => i !== idx);
      return { ...prev, images: updated, image: updated[0] || '' };
    });
  };
  ...
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-3xl font-black text-fuji-blue tracking-tighter uppercase">Thư Viện Công Trình</h2>
          <p className="text-xs text-slate-400 mt-1 uppercase font-black tracking-widest italic">Quản lý dự án thực tế đã thi công</p>
        </div>
        <button 
          onClick={() => setIsEditing({ id: 0, title: '', location: '', type: 'Homelift', description: '', images: [], image: '' })}
          className="px-6 py-3 bg-fuji-accent text-white rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl flex items-center gap-2 group hover:bg-fuji-blue transition-all"
        >
      ...
      <div className="grid md:grid-cols-3 gap-6">
        {projects.map(project => (
          <div key={project.id} className="bg-white rounded-[30px] p-4 shadow-sm border border-slate-100 group">
            <div className="h-48 rounded-[20px] overflow-hidden relative mb-4">
              <img src={project.images?.[0] || project.image} className="w-full h-full object-cover" />
              <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                 <button onClick={() => setIsEditing(project)} className="w-8 h-8 bg-white text-fuji-blue rounded-lg flex items-center justify-center shadow-lg hover:bg-fuji-blue hover:text-white"><Edit2 size={14} /></button>
                 <button onClick={async () => {
                    if(confirm('Xóa công trình này?')) {
                      const newProjects = projects.filter(p => p.id !== project.id);
                      setProjects(newProjects);
                      const { data } = await supabase.from('site_settings').select('content_dict').eq('id', 'default').single();
                      await supabase.from('site_settings').update({ content_dict: { ...data.content_dict, projects: newProjects } }).eq('id', 'default');
                    }
                 }} className="w-8 h-8 bg-white text-red-500 rounded-lg flex items-center justify-center shadow-lg hover:bg-red-500 hover:text-white"><Trash2 size={14} /></button>
              </div>
            </div>
            <div className="px-2">
              <span className="text-[9px] font-black text-fuji-accent uppercase tracking-widest">{project.type}</span>
              <h3 className="text-base font-black text-fuji-blue leading-tight mt-1 mb-2 line-clamp-1">{project.title}</h3>
              <p className="text-[10px] text-slate-400 font-bold flex items-center gap-1"><MapPin size={10} /> {project.location}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      {isEditing && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-fuji-blue/80 backdrop-blur-md">
           <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white rounded-[40px] p-10 max-w-3xl w-full max-h-[90vh] overflow-y-auto shadow-2xl relative">
              <button onClick={() => setIsEditing(null)} className="absolute top-8 right-8 text-slate-300 hover:text-fuji-blue"><X size={24} /></button>
              <h3 className="text-2xl font-black text-fuji-blue uppercase tracking-tighter mb-8">Chỉnh sửa Công trình</h3>
            ...
            setIsSaving(true);
                try {
                  ...
                  if (updated.id === 0) updated.id = Date.now();
                  const newProjects = isEditing.id === 0 ? [updated, ...projects] : projects.map(p => p.id === updated.id ? updated : p);
            ...
            } catch(err) { alert('Lỗi khi lưu!'); } finally {
              setIsSaving(false);
            }
              }} className="space-y-6">
                 <Input name="title" label="Tên công trình" defaultValue={isEditing.title} required />
                 <div className="grid grid-cols-2 gap-6">
                    <Input name="location" label="Địa điểm (VD: Hà Nội)" defaultValue={isEditing.location} required />
                    <Input name="type" label="Loại thang máy" defaultValue={isEditing.type} required />
                 </div>
                 <Textarea name="description" label="Mô tả công trình" defaultValue={isEditing.description} rows={4} />
                 
                 <div className="space-y-2 border-t pt-4">
                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-4 mb-1 block">Thư viện ảnh (Tối đa 10 ảnh)</label>
                    <div className="flex flex-col gap-4 bg-slate-50 p-4 rounded-2xl border border-slate-100">
                      { (isEditing.images && isEditing.images.length > 0) && (
                        <div className="flex flex-wrap gap-3">
                          {isEditing.images.map((img: string, idx: number) => (
                            <div key={idx} className="relative group">
                              <img src={img} className="w-20 h-20 rounded-xl object-cover border-2 border-white shadow-sm shrink-0" />
                              <div className="absolute -top-2 -right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button type="button" onClick={() => setCropImage({ url: img, onSave: async (blob: Blob) => {
                                    const isMock = !import.meta.env.VITE_SUPABASE_URL; if (isMock) { setIsEditing((prev: any) => { const newImgs = [...prev.images]; newImgs[idx] = URL.createObjectURL(blob); return {...prev, images: newImgs}; }); setCropImage(null); return; }
                                    const file = new File([blob], `crop-${Date.now()}.jpg`, { type: 'image/jpeg' }); const fileName = `proj-crop-${Date.now()}.jpg`;
                                    const { error } = await supabase.storage.from('images').upload(fileName, file);
                                    if (!error) { const url = supabase.storage.from('images').getPublicUrl(fileName).data.publicUrl; setIsEditing((prev: any) => { const newImgs = [...prev.images]; newImgs[idx] = url; return {...prev, images: newImgs}; }); }
                                    setCropImage(null);
                                }})} className="w-6 h-6 bg-fuji-blue text-white rounded-full flex items-center justify-center shadow-md hover:bg-fuji-accent"><CropIcon size={12} /></button>
                                <button type="button" onClick={() => removeImage(idx)} className="w-6 h-6 bg-red-500 text-white rounded-full flex items-center justify-center shadow-md hover:bg-red-600"><X size={12} /></button>
                              </div>
                            </div>
                          ))}
                        </div>
                      )}
                      {(!isEditing.images || isEditing.images.length < 10) && <input type="file" multiple accept="image/*" onChange={handleImageUpload} className="w-full text-xs font-bold text-slate-500 cursor-pointer" />}
                    </div>
                 </div>
             <button type="submit" disabled={isSaving} className="w-full py-5 rounded-2xl bg-fuji-blue text-white font-black text-xs uppercase tracking-widest shadow-xl hover:bg-fuji-accent transition-all mt-4 disabled:opacity-50">
               {isSaving ? 'Đang lưu...' : 'Lưu Công Trình'}
             </button>
              </form>
           </motion.div>
        </div>
...
function ConfiguratorManager({ setCropImage }: { setCropImage: any }) {
    ...
    <div className="space-y-8">
        ...
        {configs.map((config) => (
          <div key={config.id} className="bg-white rounded-[30px] p-6 shadow-sm border border-slate-100 group relative">
            ...
            <div className="h-48 rounded-2xl overflow-hidden mb-6 relative">
              {config.bg ? (
                <img src={config.bg} alt={config.name} onError={(e) => { e.currentTarget.src = 'https://images.unsplash.com/photo-1600585154340-be6161a56a0c?auto=format&fit=crop&q=80&w=1200' }} className="w-full h-full object-cover" />
              ) : (
      ...
      {isEditing && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-fuji-blue/80 backdrop-blur-md">
                ...
                <div className="space-y-1">
                  ...
                  {isEditing.bg && (
                    <div className="relative group inline-block mt-2">
                      <img src={isEditing.bg} alt="Preview" className="h-20 w-auto rounded-xl object-cover border-2 border-white shadow-sm" />
                      <button type="button" onClick={() => setCropImage({ url: isEditing.bg, onSave: async (blob: Blob) => {
                          const isMock = !import.meta.env.VITE_SUPABASE_URL;
                          if (isMock) { setIsEditing((prev: any) => ({ ...prev, bg: URL.createObjectURL(blob) })); setCropImage(null); return; }
                          const file = new File([blob], `config-crop.jpg`, { type: 'image/jpeg' }); const fileName = `config-crop-${Date.now()}.jpg`;
                          const { error } = await supabase.storage.from('images').upload(fileName, file);
                          if (!error) { const url = supabase.storage.from('images').getPublicUrl(fileName).data.publicUrl; setIsEditing((prev: any) => ({ ...prev, bg: url })); }
                          setCropImage(null);
                      }})} className="absolute -top-2 -right-2 w-6 h-6 bg-fuji-blue text-white rounded-full flex items-center justify-center shadow-md opacity-0 group-hover:opacity-100 transition-opacity hover:bg-fuji-accent">
                        <CropIcon size={12} />
                      </button>
                    </div>
                  )}
          ...
          <button type="submit" disabled={isSaving} className="w-full py-5 rounded-2xl bg-fuji-blue text-white font-black text-xs uppercase tracking-widest shadow-xl shadow-fuji-blue/20 hover:bg-fuji-accent transition-all active:scale-95 mt-4 disabled:opacity-50">
...
function WarrantyManager() {
  ...
  const [monthsOverall, setMonthsOverall] = React.useState('12 Tháng');
  const [yearsMotor, setYearsMotor] = React.useState('5 Năm');
  const [yearsCable, setYearsCable] = React.useState('10 Năm');
  const [isSaving, setIsSaving] = React.useState(false);
  ...
  React.useEffect(() => {
    const loadContent = async () => {
      ...
      if (data?.content_dict?.warranty) {
        ...
        setMonthsOverall(data.content_dict.warranty.monthsOverall || '12 Tháng');
        setYearsMotor(data.content_dict.warranty.yearsMotor || '5 Năm');
        setYearsCable(data.content_dict.warranty.yearsCable || '10 Năm');
      }
    };
    loadContent();
  }, []);
  ...
  const handleSave = async () => {
    ...
    try {
      const { data } = await supabase.from('site_settings').select('content_dict').eq('id', 'default').single();
      const currentDict = data?.content_dict || {};
      currentDict.warranty = { content, monthsOverall, yearsMotor, yearsCable };
      
      await supabase.from('site_settings').update({ content_dict: currentDict }).eq('id', 'default');
      alert('Đã cập nhật chính sách và cam kết chất lượng!');
    } catch (err) {
      console.error(err);
      alert('Có lỗi khi lưu!');
    } finally {
  ...
  return (
    <div className="bg-white rounded-[50px] p-12 border shadow-sm max-w-4xl mx-auto">
       ...
       <div className="space-y-6">
          <div className="grid md:grid-cols-3 gap-6 p-8 bg-fuji-blue rounded-[30px] shadow-lg">
             <Input label="Bảo hành toàn diện (Tổng thể)" value={monthsOverall} onChange={(e: any) => setMonthsOverall(e.target.value)} className="w-full px-6 py-4 bg-white/10 text-white rounded-2xl border border-white/10 outline-none font-bold" />
             <Input label="Bảo hành Cụm động cơ" value={yearsMotor} onChange={(e: any) => setYearsMotor(e.target.value)} className="w-full px-6 py-4 bg-white/10 text-white rounded-2xl border border-white/10 outline-none font-bold" />
             <Input label="Bảo hành Ray & Cáp tải" value={yearsCable} onChange={(e: any) => setYearsCable(e.target.value)} className="w-full px-6 py-4 bg-white/10 text-white rounded-2xl border border-white/10 outline-none font-bold" />
          </div>
          
          <Textarea 
            label="Chi tiết điều khoản (Hiển thị như một văn bản chính thức)"
            value={content}
            onChange={(e: any) => setContent(e.target.value)}
            rows={12}
            placeholder="Nhập nội dung chính sách bảo hành chi tiết..."
          />
          <div className="flex justify-end items-center bg-slate-50 p-6 rounded-3xl border border-slate-100">
             <button 
               onClick={handleSave}
               disabled={isSaving}
...
function SettingsManager() {
  ...
  React.useEffect(() => {
    const loadConfigs = async () => {
      ...
      if (data?.content_dict?.theme_settings) {
        const theme = data.content_dict.theme_settings;
        if (theme.font) setFont(theme.font);
        if (theme.accent) setAccent(theme.accent);
        if (theme.lineHeight) setLineHeight(theme.lineHeight);
      }
    };
    loadConfigs();
  }, []);
  ...
  const handleSaveTheme = async () => {
    ...
    try {
      const { data } = await supabase.from('site_settings').select('content_dict').eq('id', 'default').single();
      const currentDict = data?.content_dict || {};
      currentDict.theme_settings = { font, accent, lineHeight };
      
      await supabase.from('site_settings').update({ content_dict: currentDict }).eq('id', 'default');
      alert('Lưu cấu hình Giao diện thành công! Vui lòng tải lại website để xem thay đổi.');
    } catch (error) {
      console.error(error);
      alert('Có lỗi khi lưu cấu hình.');
    } finally {
  ...
  const handleResetLeads = async (type: 'all' | 'completed') => {
    const confirmMsg = type === 'all' 
      ? "⚠️ CẢNH BÁO: XÓA TOÀN BỘ DỮ LIỆU?\nBạn đang chuẩn bị xóa sạch danh sách khách hàng (Báo giá & Ứng tuyển) trực tiếp từ máy chủ. Nhập chữ 'RESET' (viết hoa) để xác nhận:"
      : "⚠️ XÓA KHÁCH HÀNG CŨ?\nHành động này sẽ xóa các khách hàng có trạng thái 'Hoàn thành' hoặc 'Đã hủy'. Nhập chữ 'RESET' để xác nhận:";
    ...
    if (pass === 'RESET') {
      ...
      try {
        ...
        alert('✅ Đã dọn dẹp Supabase thành công! Máy chủ đã được giải phóng giúp Admin tải nhanh hơn.');
      } catch (e: any) {
      ...
      alert('Xác nhận không hợp lệ. Đã hủy thao tác.');
    }
  };
    ...
    <div className="max-w-4xl mx-auto space-y-8">
      ...
      <div className="bg-white rounded-[40px] p-12 shadow-sm border border-slate-100">
           ...
           <div className="space-y-6">
              <div className="space-y-3">
                 ...
                 <select value={font} onChange={(e) => setFont(e.target.value)} className="w-full px-6 py-5 rounded-2xl bg-slate-50 border-none font-bold text-fuji-blue outline-none">
                    ...
                    <option value="Nunito">Sharp (Nunito)</option>
                 </select>
              </div>
           </div>
           <div className="space-y-6">
              <div className="space-y-3">
                 <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Độ giãn dòng (Line Height)</label>
                 <select value={lineHeight} onChange={(e) => setLineHeight(e.target.value)} className="w-full px-6 py-5 rounded-2xl bg-slate-50 border-none font-bold text-fuji-blue outline-none">
                    <option value="1.4">Gọn gàng (1.4)</option>
                    <option value="1.6">Tiêu chuẩn (1.6)</option>
                    <option value="1.8">Thoáng đãng (1.8)</option>
                    <option value="2.0">Rộng rãi (2.0)</option>
                 </select>
              </div>
              <div className="space-y-3">
        ...
        <div className="mt-8 flex justify-end">
           <button onClick={handleSaveTheme} disabled={isSaving} className="px-8 py-4 bg-fuji-accent text-white rounded-xl text-[10px] font-black uppercase tracking-widest shadow-xl hover:bg-fuji-blue transition-all disabled:opacity-50">
      ...
      <div className="bg-white rounded-[40px] p-12 shadow-sm border border-red-100 relative overflow-hidden">
        <div className="absolute top-0 right-0 w-64 h-64 bg-red-50 rounded-full blur-[80px] pointer-events-none" />
        <h3 className="text-xl font-black text-red-600 mb-8 uppercase tracking-tight flex items-center gap-3 relative z-10">
          <Trash2 size={20} /> Tối ưu hóa & Dọn dẹp Máy chủ
        </h3>
        <p className="text-sm font-medium text-slate-500 mb-8 relative z-10 max-w-2xl">
          Việc lưu trữ quá nhiều dữ liệu khách hàng cũ có thể làm giảm tốc độ tải trang của bảng quản trị. Tính năng này giúp bạn xóa sạch (Reset) dữ liệu trực tiếp từ Supabase để web tải mượt mà hơn.
        </p>
        <div className="flex flex-wrap gap-4 relative z-10">
          <button 
            onClick={() => handleResetLeads('completed')} 
            disabled={isSaving}
            className="px-6 py-4 bg-orange-50 text-orange-600 rounded-xl text-xs font-black uppercase tracking-widest hover:bg-orange-100 transition-all disabled:opacity-50 flex items-center gap-2"
          >
            <Trash2 size={16} /> Dọn dẹp Data đã xử lý
          </button>
          <button 
            onClick={() => handleResetLeads('all')} 
            disabled={isSaving}
            className="px-6 py-4 bg-red-500 text-white rounded-xl text-xs font-black uppercase tracking-widest hover:bg-red-600 shadow-xl shadow-red-500/20 transition-all disabled:opacity-50 flex items-center gap-2"
          >
            <AlertCircle size={16} /> Reset TOÀN BỘ Supabase
          </button>
        </div>
      </div>
    </div>
...
function WebContentManager({ setCropImage }: { setCropImage: any }) {
  ...
  return (
    <form onSubmit={handleSave} className="max-w-4xl mx-auto space-y-8">
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-slate-50">
             <div className="space-y-2 col-span-2 md:col-span-1">
               ...
               <p className="text-[9px] text-slate-400 ml-4 mt-1">Hỗ trợ định dạng PNG, JPG, SVG (khuyên dùng SVG cho logo xóa phông).</p>
               {content.logoImage && (
                 <div className="relative group inline-block mt-2">
                   <img src={content.logoImage} alt="Logo" className="h-16 w-auto object-contain p-2 bg-slate-100 rounded-xl" />
                   <button type="button" onClick={() => setCropImage({ url: content.logoImage!, onSave: async (blob: Blob) => {
                      const file = new File([blob], `logo-crop.png`, { type: 'image/png' }); const fileName = `web-logo-${Date.now()}.png`;
                      if (!import.meta.env.VITE_SUPABASE_URL) { setContent(prev => ({...prev, logoImage: URL.createObjectURL(blob)})); setCropImage(null); return; }
                      const { error } = await supabase.storage.from('images').upload(fileName, file);
                      if (!error) { const url = supabase.storage.from('images').getPublicUrl(fileName).data.publicUrl; setContent(prev => ({...prev, logoImage: url})); } setCropImage(null);
                   }})} className="absolute -top-2 -right-2 w-6 h-6 bg-fuji-blue text-white rounded-full flex items-center justify-center shadow-md opacity-0 group-hover:opacity-100 transition-opacity hover:bg-fuji-accent"><CropIcon size={12} /></button>
                 </div>
               )}
             </div>
             <div className="space-y-2">
               ...
               {content.heroImage && (
                 <div className="relative group inline-block mt-2">
                   <img src={content.heroImage} alt="Hero" className="h-24 w-auto rounded-xl object-cover border-2 border-white shadow-sm" />
                   <button type="button" onClick={() => setCropImage({ url: content.heroImage!, onSave: async (blob: Blob) => {
                      const file = new File([blob], `hero-crop.jpg`, { type: 'image/jpeg' }); const fileName = `web-hero-${Date.now()}.jpg`;
                      if (!import.meta.env.VITE_SUPABASE_URL) { setContent(prev => ({...prev, heroImage: URL.createObjectURL(blob)})); setCropImage(null); return; }
                      const { error } = await supabase.storage.from('images').upload(fileName, file);
                      if (!error) { const url = supabase.storage.from('images').getPublicUrl(fileName).data.publicUrl; setContent(prev => ({...prev, heroImage: url})); } setCropImage(null);
                   }})} className="absolute -top-2 -right-2 w-6 h-6 bg-fuji-blue text-white rounded-full flex items-center justify-center shadow-md opacity-0 group-hover:opacity-100 transition-opacity hover:bg-fuji-accent"><CropIcon size={12} /></button>
                 </div>
               )}
             </div>
             <div className="space-y-2">
               ...
               {content.aboutImage && (
                 <div className="relative group inline-block mt-2">
                   <img src={content.aboutImage} alt="About" className="h-24 w-auto rounded-xl object-cover border-2 border-white shadow-sm" />
                   <button type="button" onClick={() => setCropImage({ url: content.aboutImage!, onSave: async (blob: Blob) => {
                      const file = new File([blob], `about-crop.jpg`, { type: 'image/jpeg' }); const fileName = `web-about-${Date.now()}.jpg`;
                      if (!import.meta.env.VITE_SUPABASE_URL) { setContent(prev => ({...prev, aboutImage: URL.createObjectURL(blob)})); setCropImage(null); return; }
                      const { error } = await supabase.storage.from('images').upload(fileName, file);
                      if (!error) { const url = supabase.storage.from('images').getPublicUrl(fileName).data.publicUrl; setContent(prev => ({...prev, aboutImage: url})); } setCropImage(null);
                   }})} className="absolute -top-2 -right-2 w-6 h-6 bg-fuji-blue text-white rounded-full flex items-center justify-center shadow-md opacity-0 group-hover:opacity-100 transition-opacity hover:bg-fuji-accent"><CropIcon size={12} /></button>
                 </div>
               )}
             </div>
      ...
      <div className="flex justify-end">
        <button type="submit" disabled={isSaving} className="px-10 py-4 bg-fuji-blue text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl hover:bg-fuji-accent transition-all active:scale-95 disabled:opacity-50">
          {isSaving ? 'Đang lưu...' : 'Lưu thay đổi nội dung'}
        </button>
      </div>
    </form>
  );
...
function NewsManager({ setCropImage }: { setCropImage: any }) {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
        ...
        <h4 className="font-bold text-slate-800 text-xs tracking-tighter leading-tight mb-1 line-clamp-2">{post.title}</h4>
        <p className="text-[10px] text-slate-500 leading-snug line-clamp-3">{post.summary}</p>
          </div>
        ))}
      </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-fuji-blue/80 backdrop-blur-md">
          <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-white rounded-[40px] p-10 max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl relative">
            ...
            <form onSubmit={handleSave} className="space-y-6">
              ...
              <RichTextEditor name="content" label="Nội dung chi tiết (Trình soạn thảo chuyên nghiệp)" defaultValue={isEditing.content} uploadedImages={isEditing.images} />
              <div className="space-y-1">
                    ...
                    <div className="flex flex-wrap gap-3">
                      {isEditing.images.map((img, idx) => (
                        <div key={idx} className="relative group">
                          ...
                          <div className="absolute -top-2 -right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button type="button" onClick={() => setCropImage({ url: img, onSave: async (blob: Blob) => {
                                const isMock = !import.meta.env.VITE_SUPABASE_URL; if (isMock) { setIsEditing((prev: any) => { const newImgs = [...prev.images]; newImgs[idx] = URL.createObjectURL(blob); return {...prev, images: newImgs, imageUrl: newImgs[0]}; }); setCropImage(null); return; }
                                const file = new File([blob], `crop-${Date.now()}.jpg`, { type: 'image/jpeg' }); const fileName = `news-crop-${Date.now()}.jpg`;
                                const { error } = await supabase.storage.from('images').upload(fileName, file);
                                if (!error) { const url = supabase.storage.from('images').getPublicUrl(fileName).data.publicUrl; setIsEditing((prev: any) => { const newImgs = [...prev.images]; newImgs[idx] = url; return {...prev, images: newImgs, imageUrl: newImgs[0]}; }); }
                                setCropImage(null);
                            }})} className="w-6 h-6 bg-fuji-blue text-white rounded-full flex items-center justify-center shadow-md hover:bg-fuji-accent"><CropIcon size={12} /></button>
                            <button type="button" onClick={() => removeImage(idx)} className="w-6 h-6 bg-red-500 text-white rounded-full flex items-center justify-center shadow-md hover:bg-red-600">
                              <X size={12} />
              ...
              {isEditing.comments && isEditing.comments.length > 0 && (
                <div className="space-y-4 border-t border-slate-200 pt-8 mt-8">
                  <p className="text-[10px] font-black uppercase tracking-widest text-fuji-accent border-b pb-2 mb-4">Quản lý Bình luận của Khách hàng</p>
                  {isEditing.comments.map((comment: any) => (
                    <div key={comment.id} className="p-5 bg-slate-50 rounded-2xl flex justify-between items-start gap-4 border border-slate-100">
                      <div>
                        <p className="font-bold text-sm text-fuji-blue">{comment.name} <span className="text-[11px] text-slate-400 font-medium ml-2">({new Date(comment.createdAt).toLocaleString('vi-VN')})</span></p>
                        <p className="text-sm text-slate-700 mt-2 italic border-l-2 border-slate-300 pl-3">{comment.content}</p>
                        {!comment.isApproved && <span className="inline-block mt-3 px-3 py-1 bg-orange-100 text-orange-600 text-[10px] font-black rounded-lg uppercase tracking-wider">Đang chờ duyệt</span>}
                      </div>
                      <div className="flex gap-2 shrink-0">
                        {!comment.isApproved && (
                          <button type="button" title="Phê duyệt" onClick={() => handleApproveComment(comment.id)} className="w-9 h-9 bg-white border border-green-200 text-green-600 rounded-xl flex items-center justify-center hover:bg-green-500 hover:text-white transition-all shadow-sm"><CheckCircle2 size={16} /></button>
                        )}
                        <button type="button" title="Xóa bình luận" onClick={() => handleDeleteComment(comment.id)} className="w-9 h-9 bg-white border border-red-200 text-red-600 rounded-xl flex items-center justify-center hover:bg-red-500 hover:text-white transition-all shadow-sm"><Trash2 size={16} /></button>
                      </div>
                    </div>
                  ))}
                </div>
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isEditing.id) { // Update
        ...
        if (!newPassword) { alert('Vui lòng nhập mật khẩu cho tài khoản mới.'); setIsSaving(false); return; }
...
// Renamed StatCard to DashboardStatCard and added onClick prop
function DashboardStatCard({ icon, label, value, trend, positive, onClick }: { icon: React.ReactNode; label: string; value: string | number; trend: string; positive: boolean; onClick?: () => void }) {
  return (
    <div 
      onClick={onClick}
      className={cn(
        "bg-white rounded-[32px] p-8 shadow-sm border border-slate-100 hover:shadow-xl hover:-translate-y-1 transition-all group",
...
function RichTextEditor({ name, label, defaultValue, uploadedImages = [] }: { name: string, label: string, defaultValue?: string, uploadedImages?: string[] }) {
  ...
  return (
    <div className="space-y-2">
       <div className="flex justify-between items-end mb-2">
         <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-4 block">{label}</label>
         <div className="flex bg-slate-100 p-1 rounded-xl">
           <button type="button" onClick={() => setMode('edit')} className={cn("px-5 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all", mode === 'edit' ? "bg-white text-fuji-blue shadow-sm" : "text-slate-400 hover:text-fuji-blue")}>Soạn thảo</button>
           <button type="button" onClick={() => setMode('preview')} className={cn("px-5 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all", mode === 'preview' ? "bg-white text-fuji-blue shadow-sm" : "text-slate-400 hover:text-fuji-blue")}>Xem trước</button>
         </div>
       </div>
       
       <div className="bg-slate-50 rounded-[20px] border border-slate-200 overflow-hidden focus-within:ring-2 focus-within:ring-fuji-accent/20 transition-all shadow-inner">
          {mode === 'edit' ? (
            <>
              <div className="flex flex-wrap gap-2 p-2 bg-white border-b border-slate-200 items-center sticky top-0 z-10 shadow-sm">
                <div className="flex bg-slate-50 rounded-lg p-1 border border-slate-100">
                  <button type="button" onClick={() => insertText('**', '**')} className="w-9 h-9 flex items-center justify-center rounded-md hover:bg-white hover:shadow-sm text-slate-700 transition-all" title="In đậm"><Bold size={16} strokeWidth={3} /></button>
                  <button type="button" onClick={() => insertText('*', '*')} className="w-9 h-9 flex items-center justify-center rounded-md hover:bg-white hover:shadow-sm text-slate-700 transition-all" title="In nghiêng"><Italic size={16} strokeWidth={3} /></button>
                  <button type="button" onClick={() => insertText('__', '__')} className="w-9 h-9 flex items-center justify-center rounded-md hover:bg-white hover:shadow-sm text-slate-700 transition-all" title="Gạch chân"><Underline size={16} strokeWidth={3} /></button>
                </div>
                <div className="w-px h-6 bg-slate-200 mx-1" />
                <div className="flex bg-slate-50 rounded-lg p-1 border border-slate-100 gap-1">
                  <button type="button" onClick={() => insertText('1. ')} className="w-10 h-9 flex items-center justify-center rounded-md hover:bg-white hover:shadow-sm text-slate-700 transition-all" title="Tiêu đề Lớn"><Heading1 size={18} /></button>
                  <button type="button" onClick={() => insertText('1.1. ')} className="w-10 h-9 flex items-center justify-center rounded-md hover:bg-white hover:shadow-sm text-slate-700 transition-all" title="Tiêu đề Vừa"><Heading2 size={18} /></button>
                  <button type="button" onClick={() => insertText('> ')} className="w-10 h-9 flex items-center justify-center rounded-md hover:bg-white hover:shadow-sm text-slate-700 transition-all gap-1" title="Trích dẫn đoạn văn"><Quote size={16} /></button>
                </div>
                <div className="w-px h-6 bg-slate-200 mx-1" />
                <div className="flex bg-fuji-blue/5 rounded-lg p-1 border border-fuji-blue/10 gap-1">
                  <button type="button" onClick={() => insertText('\n[img:1] Chú thích ảnh\n', '')} className="px-4 h-9 flex items-center justify-center rounded-md text-[10px] font-black uppercase tracking-widest text-fuji-blue hover:bg-white hover:shadow-sm transition-all" title="Chèn vị trí Ảnh 1">+ Ảnh 1</button>
                  <button type="button" onClick={() => insertText('[img:2] Chú thích ảnh\n', '')} className="px-4 h-9 flex items-center justify-center rounded-md text-[10px] font-black uppercase tracking-widest text-fuji-blue hover:bg-white hover:shadow-sm transition-all" title="Chèn vị trí Ảnh 2">+ Ảnh 2</button>
                  <button type="button" onClick={() => insertText('[img:3] Chú thích ảnh\n', '')} className="px-4 h-9 flex items-center justify-center rounded-md text-[10px] font-black uppercase tracking-widest text-fuji-blue hover:bg-white hover:shadow-sm transition-all" title="Chèn vị trí Ảnh 3">+ Ảnh 3</button>
                </div>
              </div>
              <textarea ref={textareaRef} name={name} value={val} onChange={(e) => setVal(e.target.value)} rows={20} className="w-full px-8 py-8 bg-slate-50/50 border-none outline-none font-medium text-slate-700 resize-y leading-[1.8]" placeholder="Soạn thảo nội dung ở đây..." />
            </>
          ) : (
              ...
              <div className="max-w-3xl mx-auto">
                ...
                {val.split('\n').map((line, idx) => {
                    ...
                    if (imgMatch) {
                      ...
                      const imgSrc = uploadedImages[imgIndex];
                      
                      if (imgSrc) {
                        return (
                          <figure key={idx} className="my-10 flex flex-col items-center">
                            <img src={imgSrc} alt={caption || 'Preview'} className="w-full rounded-[30px] shadow-2xl object-cover" />
                            {caption && <figcaption className="text-center text-sm font-medium text-slate-500 italic mt-6 px-6 border-l-2 border-fuji-accent py-1">{caption}</figcaption>}
                          </figure>
                        );
                      }
                      return <div key={idx} className="my-10 p-12 border-2 border-dashed border-slate-300 rounded-[30px] flex flex-col items-center justify-center text-slate-400 bg-slate-50"><ImageIcon size={40} className="mb-3 opacity-50" /><span className="text-sm font-black uppercase tracking-widest">Chưa có Ảnh {imgIndex + 1}</span><span className="text-xs mt-2 font-medium">Vui lòng upload ảnh bên dưới hoặc đổi mã [img:...]</span></div>;
                    }

                    if (trimmed.startsWith('>')) {
                      return (
                        <blockquote key={idx} className="my-10 pl-6 md:pl-10 border-l-4 border-fuji-accent bg-gradient-to-r from-fuji-accent/5 to-transparent py-8 pr-8 rounded-r-[30px] relative">
                          <Quote size={40} className="text-fuji-accent/20 absolute top-4 left-4" />
                          <p className="text-xl md:text-2xl italic font-medium text-slate-800 leading-[1.6] relative z-10">{trimmed.substring(1).trim()}</p>
                        </blockquote>
                      );
                    }

                    if (/^\d+\.\s+/.test(trimmed)) return <h3 key={idx} className="text-2xl md:text-4xl font-black text-fuji-blue mt-12 mb-6 leading-tight tracking-tighter">{trimmed}</h3>;
                    if (/^\d+\.\d+\.\s+/.test(trimmed)) return <h4 key={idx} className="text-xl md:text-2xl font-bold text-fuji-blue mt-8 mb-4 leading-tight tracking-tight">{trimmed}</h4>;
                    
                    return <p key={idx} className="text-slate-800 text-lg leading-relaxed tracking-tight mb-6">{renderText(trimmed)}</p>;
                })}
              </div>
            </div>
          )}
...
function ImageCropperModal({ imageUrl, onSave, onCancel }: { imageUrl: string, onSave: (blob: Blob) => Promise<void>, onCancel: () => void }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const canvas = document.createElement('canvas'); canvas.width = (croppedAreaPixels as any).width; canvas.height = (croppedAreaPixels as any).height;
      const ctx = canvas.getContext('2d');
      if (ctx) {
        ctx.drawImage(image, (croppedAreaPixels as any).x, (croppedAreaPixels as any).y, (croppedAreaPixels as any).width, (croppedAreaPixels as any).height, 0, 0, (croppedAreaPixels as any).width, (croppedAreaPixels as any).height);
        canvas.toBlob(async blob => { if (blob) { await onSave(blob); } else { setIsSaving(false); } }, 'image/jpeg', 0.95);
      }
    } catch(e) {
      console.error(e); alert('Lỗi tải ảnh do chính sách bảo mật CORS của server cũ. Hãy chọn ảnh mới từ máy tính của bạn để Crop.'); setIsSaving(false);
    }
  }
  ...
  return (
    <div className="fixed inset-0 z-[200] bg-slate-900/95 backdrop-blur-sm flex flex-col items-center justify-center p-4 md:p-10">
       ...
       <div className="flex flex-col gap-4 w-full max-w-lg bg-white/10 backdrop-blur-md p-8 rounded-[40px] border border-white/10 shadow-2xl">
          ...
          <input type="range" min={1} max={3} step={0.01} value={zoom} onChange={e => setZoom(Number(e.target.value))} className="w-full accent-fuji-accent h-2 bg-white/20 rounded-full appearance-none cursor-pointer" />
          <div className="flex gap-4 mt-4">
            <button onClick={onCancel} disabled={isSaving} className="flex-1 py-4 bg-white/5 text-white/70 rounded-2xl font-black uppercase text-[10px] tracking-widest hover:bg-white/10 hover:text-white transition-all">Hủy</button>
            <button onClick={handleSave} disabled={isSaving} className="flex-[2] py-4 bg-fuji-accent text-white rounded-2xl font-black uppercase text-[10px] tracking-widest hover:bg-fuji-blue transition-all flex items-center justify-center gap-2 shadow-xl shadow-fuji-accent/20">
              {isSaving ? 'Đang xử lý...' : 'Cắt & Lưu ảnh'}
            </button>
          </div>
       </div>
    </div>
